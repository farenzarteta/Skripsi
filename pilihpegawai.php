                              ...
                              while ($row=mysqli_fetch_assoc($res))
                              {
                                ?>
                                  <tr>
                                      ...
                                      <td>
                                        <a href="evaluasistaf.php?id_staf=<?php echo $row["ID_STAF"];?>&periode=<?php echo $_GET["id"];?>">
                                          Evaluasi
                                        </a>
                                        <?php
                                          $q="SELECT * FROM evaluasi_kinerja WHERE ID_STAF='". $row["ID_STAF"] ."' AND ID_PERIODE='". $_GET["id"] ."'";
                                          $res2=mysqli_query($con,$q);
                                          if ($row=mysqli_fetch_assoc($res2))
                                          {
                                            ?>
                                              | <a href="lihatpenilaian.php?id=<?php echo $row["ID_EVALUASI_KINERJA"];?>">Lihat Penilaian</a>
                                            <?php
                                          }
                                        ?>
                                      </td>
