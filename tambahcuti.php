...
<div class="inner-block">
    <div class="blank">
      <div class="blankpage-main">
        ...
        <form id="formData" method="POST" action="simpancuti.php">
                <div class="form-group">
                  <input readonly value="<?php echo $dataUser["ID_STAF"]; ?>" type="text" class="form-control" id="id_staf" name="id_staf">
                </div>
                <div class="form-group">
                  <label for="email">Tanggal mulai cuti:</label>
                  <input type="date" class="form-control" id="tgl_mulai_cuti" name="tgl_mulai_cuti">
                </div>
                <div class="form-group">
                  <label for="email">Tanggal selesai cuti:</label>
                  <input type="date" class="form-control" id="tgl_selesai_cuti" name="tgl_selesai_cuti">
                </div>
                <div class="form-group">
                  <label for="email">Alasan cuti:</label>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="periode">
                      <?php
                            $q="SELECT *,DATE_FORMAT(TGL_START_PER,'%d-%M-%Y') as tstart,DATE_FORMAT(TGL_FINISH_PER,'%d-%M-%Y') as tend FROM periode";
                            $res=mysqli_query($con,$q);
                            while ($row=mysqli_fetch_assoc($res))
                            {
                              ?>
                                <option value="<?php echo $row["ID_PER"];?>">
                                    <?php
                                          echo $row["NAMA_PER"]." - ".$row["tstart"]." - ".$row["tend"];
                                    ?>
                                </option>
                              <?php
                            }
...
?>
  <script>
        ...
        function hitunghari()
        {
              var tanggal=new Date($("#tgl_mulai_cuti").val());
              var tanggal2=new Date($("#tgl_selesai_cuti").val());
              var timeDiff = Math.abs(tanggal2.getTime() - tanggal.getTime()+1);
        ...
        $(document).ready(function()
        {
            ...
            $("#tgl_selesai_cuti").change(function()
              {
                hitunghari();
              });
        });
